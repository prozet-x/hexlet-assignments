...
class AppTest {
    ...
    // BEGIN
    @Test
    void testCreateUserWithCorrectData() throws IOException, ParseException {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpPost postRequest = new HttpPost(baseUrl + "/users/new");

        List<NameValuePair> params = new ArrayList<>();

        params.add(new BasicNameValuePair("firstName", "1"));
        params.add(new BasicNameValuePair("lastName", "2"));
        params.add(new BasicNameValuePair("email", "ivan1@mail.ru"));
        postRequest.setEntity(new UrlEncodedFormEntity(params));
